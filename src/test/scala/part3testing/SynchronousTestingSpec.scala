...
class SynchronousTestingSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "A word counter master" should {
    ...
    "spawn a child upon reception of the initialize message" in {
      val master: BehaviorTestKit[MasterProtocol] = BehaviorTestKit(WordCounterMaster())
    ...
    "send a task to a child" in {
      ...
      val mailbox: TestInbox[UserProtocol] = TestInbox[UserProtocol]() // the "requester"'s inbox
      // start processing
      master.run(WordCountTask("Akka testing is very powerful", mailbox.ref))
    ...
    "log messages" in {
      val master: BehaviorTestKit[MasterProtocol] = BehaviorTestKit(WordCounterMaster())
      master.run(Initialize(1)) // synchronous "sending" of the message Initialize(1)
      master.logEntries() shouldBe Seq(CapturedLogEvent(Level.INFO, "[master] Start initializing with 1 worker actors."))
    }
  }
}
